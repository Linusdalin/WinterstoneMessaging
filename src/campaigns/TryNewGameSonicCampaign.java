...
public class TryNewGameSonicCampaign extends AbstractCampaign implements CampaignInterface {

    // Campaign config data
    private static final String Game = "sonic_boom";
    private static final String GameName = "Sonic Boom";
    ...
    TryNewGameSonicCampaign(int priority, CampaignState active, String day){
    ...
    protected Reward decideReward(User user) {
        ...
        if(isHighSpender(user))
            return RewardRepository.SonicHigh;

        if(isPaying(user))
            return RewardRepository.SonicPaying;

        if(isFrequent(user))
            return RewardRepository.SonicFrequent;

        return RewardRepository.SonicRest;
    }
