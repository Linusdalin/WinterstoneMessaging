...
public class TryNewGameOS8XCampaign extends AbstractCampaign implements CampaignInterface {

    // Campaign config data
    private static final String Game = "eight_times_pay";
    private static final String GameName = "8X Special";
    ...
    TryNewGameOS8XCampaign(int priority, CampaignState active, String day){
    ...
    public static EmailInterface gameActivationEmail(User user, Reward reward, String promoCode) {
        ...
        return new NotificationEmail("We have a recommendation for you", "<p>Don't miss out one of the hottest games here on SlotAmerica. It is the "+ GameName+", with big multipliers giving massive jackpot wins.  " +
                "We do think you will like it. We have added "+ reward.getCoins()+" free spins for you to try it out!</p>" +
                "<p> Just click here <a href=\"http://smarturl.it/launch_slotamerica?game="+Game+"&promoCode="+ promoCode+"&reward="+reward.getCode() + "\"> to claim your spins</a></p>",
    ...
    protected Reward decideReward(User user) {
        ...
        if(isHighSpender(user))
            return RewardRepository.OS8XHigh;

        if(isPaying(user))
            return RewardRepository.OS8XPaying;

        if(isFrequent(user))
            return RewardRepository.OS8XFrequent;

        return RewardRepository.OS8XRest;
    }
